...
const algorithm = 'aes-256-gcm'; // Ensure this matches the encryption algorithm
...
function decryptFile(file, decryptedFile, password) {
  ...
  if (algorithm.includes('gcm')) {
    authTag = encryptedData.slice(encryptedData.length - 16);
    data = encryptedData.slice(16, encryptedData.length - 16);
  } else {
...
async function main() {
  const file = await getInput('Please provide the filepath to the file you want to decrypt: ');
  const password = await getInput('Please enter the Password: ');

  const base = file.replace('Encrypted', '');
  const decryptedFile = `${base.substring(0, base.lastIndexOf('.'))}Decrypted${base.substring(base.lastIndexOf('.'), base.length)}`;
