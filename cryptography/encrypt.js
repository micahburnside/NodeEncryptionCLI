...
const algorithm = 'aes-256-gcm'; // Specify the encryption algorithm
const algorithmCTR = 'aes-256-ctr';

async function encryptFile(file, encryptedFile, password) {
  const iv = crypto.randomBytes(16);
  const data = fs.readFileSync(file);
...
async function main() {
  ...
  await encryptFile(file, encryptedFile, password);
